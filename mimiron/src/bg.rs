...
impl From<CardData> for Card {
    fn from(c: CardData) -> Self {
        let card_type = match &c.battlegrounds {
            ...
            _ if c.card_type_id == 10 => BGCardType::HeroPower {
                text: c.text,
                cost: c.mana_cost,
            },
            ...
            _ if c.card_type_id == 44 => BGCardType::Trinket {
                ...
                trinket_kind: c
                    .spell_school_id
                    .map_or(SpellSchool::Lesser, SpellSchool::from),
            },
        ...
        let pool = c
            ...
            .and_then(|bg| {
                bg.duos_only
                    .then_some(Pool::Duos)
                    .or_else(|| bg.solos_only.then_some(Pool::Solos))
            })
            .unwrap_or_default();
...
pub fn lookup(opts: SearchOptions<'_>) -> Result<impl Iterator<Item = Card> + '_> {
    ...
    anyhow::ensure!(
        res.card_count > 0,
        "No Battlegrounds card found. Check your spelling."
    );
...
pub fn get_associated_cards(
    ...
    match &card.card_type {
        BGCardType::Hero {
            ...
            if let Some(id) = buddy_id
                && let Ok(res) = get_card_by_id(*id, locale)
            {
...
pub fn print_assoc_card(
    ...
    match (assoc, &card.card_type) {
        ...
        (
            ...
            let upgraded = format!("\t{}: {attack}/{health}", locale.golden())
                .italic()
                .yellow();
            println!("{upgraded}");
