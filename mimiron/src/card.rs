...
impl Card {
    pub(crate) fn dummy(id: usize) -> Self {
        let (name, cost, rarity) = get_hearth_sim_details(id).unwrap_or_else(|| {
            (
                format_compact!("Unknown Card ID {id}"),
                99,
                Rarity::Noncollectible,
            )
        });
    ...
    fn in_locale(
        ...
        impl Display for Inner<'_> {
            fn fmt(
                ...
                write!(
                    f,
                    "{name}{:padding$} {rarity} {class} {runes}({cost}) {faction}{card_info}.",
                    ""
                )?;
...
impl<'s> SearchOptions<'s> {
    ...
    #[must_use]
    pub const fn include_noncollectibles(
        self,
        noncollectibles: bool,
    ) -> Self {
        Self {
...
pub fn lookup(opts: SearchOptions<'_>) -> Result<impl Iterator<Item = Card> + '_> {
    ...
    anyhow::ensure!(
        ...
        fuzzed.map_or("Check your spelling".to_owned(), |s| format!(
            "Did you mean \"{}\"?",
            s.0
        ))
    );
