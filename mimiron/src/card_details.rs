...
impl<ID> Details<ID> {
    ...
    pub fn name(
        ...
        self.name
            .clone()
            .right_or_else(|ln| ln.in_locale(locale).to_compact_string())
    }
}
...
pub(crate) fn get_metadata() -> MappedRwLockReadGuard<'static, Metadata> {
    ...
    if last_update.is_none_or(|(empty, t)| empty || t.elapsed() >= REFRESH_RATE) {
        _ = METADATA
            .write()
            .insert((internal_get_metadata(), Instant::now()));
    }
...
impl Localize for SpellSchool {
    fn in_locale(
        ...
        match self {
            Self::Spellcraft | Self::Tavern | Self::Upgrade => locale.battlegrounds().into(),
            _ => get_metadata()
                .spell_schools
                .iter()
                .find(|det| *self == Self::from(det.id))
