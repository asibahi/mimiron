  ...
  fn in_locale(
    ...
    if let Some(sideboards) = &self.sideboard_cards {
      for sideboard in sideboards {
        ...
        let cards = sideboard.cards_in_sideboard.iter().fold(BTreeMap::new(), |mut map, card| {
          *map.entry(card).or_default() += 1;
          map
        });
    ...
    preceded(
      ...
      (
        ...
        (
          ...
          .map(|(v1, v2, vn)| {
            v1.into_iter().chain(v2.into_iter().flatten()).chain(vn.into_iter().flatten()).collect()
          }),
        // sideboard
        alt((
          preceded(tag([1].as_slice()), length_count(u8(), (varint(), varint()))),
          success(Vec::new()),
        )),
      )
...
mod deck_code_tests {
  ...
  test!(
    ...
    78065,
    vec![
      90648, 91251, 112846, 113663, 80647, 80647, 95344, 95344, 100101, 100101, 105855, 105855,
      107920, 107920, 110880, 110880, 111969, 111969, 112799, 112799, 112812, 112812, 112830,
      112830, 112834, 112834, 112867, 112867, 113576, 113576,
    ],
    vec![],
  );
  ...
  test!(
    ...
    7,
    vec![
      90749, 92332, 97332, 101367, 102983, 103443, 107770, 107856, 111914, 112815, 76302, 76302,
      103660, 103660, 105813, 105813, 105892, 105892, 107891, 107891, 110091, 110091, 110896,
      110896, 112856, 112856, 113211, 113211, 113212, 113212,
    ],
    vec![
      (102221, 90749),
      (104948, 102983),
