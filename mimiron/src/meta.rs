...
fn casify_archetype(at: &str) -> CompactString {
  at.split('-')
    .map(|s| {
      if s.eq_ignore_ascii_case("dk") // Death Knight
                    ...
                    ))
      {
        ...
        chars
          .next()
          .map_or_else(CompactString::default, |first| first.to_uppercase().chain(chars).collect())
      }
    })
    .fold(CompactString::default(), |acc, t| {
      if acc.is_empty() { t.to_compact_string() } else { format_compact!("{} {}", acc, t) }
    })
}

fn get_deck_from_deck_stat(
  ds: DeckStat,
  locale: Locale,
) -> Option<Deck> {
...
fn get_decks_stats(
  format: Format,
  class: Option<Class>,
) -> Result<impl Iterator<Item = DeckStat>> {
  ...
  let mut range = d_l;
  loop {
    let mut decks = get_firestone_data(range)?
      .deck_stats
      .into_iter()
      .filter(|s| s.total_games >= min_count && class.is_none_or(|c| c == s.player_class))
      .peekable();
    ...
    match decks.peek() {
      ...
      _ => {
        return Ok(decks.sorted_by(|s1, s2| {
