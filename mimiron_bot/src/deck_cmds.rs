...
fn create_deck_reply(
  deck: &Deck,
  opts: deck::ImageOptions,
) -> Result<poise::CreateReply, Error> {
  let attachment_name =
    format!("{}.png", deck.deck_code.chars().filter(|c| c.is_alphanumeric()).collect::<String>());
  ...
  if random::<u8>() % 10 == 0 {
    embed = embed.footer(serenity::CreateEmbedFooter::new("See other useful commands with /help."));
  }
...
async fn create_deck_dropdown(
  ...
  let replies = decks
    .iter()
    .map(|(_, deck)| {
      LazyCell::new(|| create_deck_reply(deck, deck::ImageOptions::Adaptable).unwrap_or_default())
    })
    .collect::<Vec<_>>();
  ...
  meta_handle
    .edit(
      ...
      poise::CreateReply::default()
        .embed(embed)
        .components(vec![serenity::CreateActionRow::SelectMenu(select_menu.disabled(true))]),
    )
    .await?;
