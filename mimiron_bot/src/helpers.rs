...
pub async fn help(ctx: Context<'_>) -> Result<(), Error> {
  ...
  if ctx.guild().is_none() && ctx.framework().options().owners.contains(&ctx.author().id) {
    let reply =
      poise::CreateReply::default().content(env!("CARGO_PKG_VERSION").to_string()).ephemeral(true);
  ...
  let embed = serenity::CreateEmbed::new().title("Help").fields(fields).footer(
    serenity::CreateEmbedFooter::new(format!("{} v:{}", FOOTER, env!("CARGO_PKG_VERSION"),)),
  );
...
pub async fn patchtime(ctx: Context<'_>) -> Result<(), Error> {
  ...
  let reply = poise::CreateReply::default()
    .content(format!("<t:{0}:F> <t:{0}:R>", patch.timestamp().as_second()));
...
pub async fn on_error(
  ...
  match error {
    poise::FrameworkError::Command { error, ctx, .. } => {
      ...
      let guild =
        ctx.guild().map_or_else(|| format!("User: {}", ctx.author().name), |g| g.name.clone());
...
pub async fn paginated_embeds<T: Send>(
  ...
  let prev_button = serenity::CreateButton::new(format!("{ctx_id}prev")).label("<").disabled(true);
