...
pub fn run_one(
  ...
  let opts =
    LookupOptions::lookup(&args.input).with_locale(locale).with_custom_format(args.mode.as_deref());
  ...
  if args.image {
    let opts = match args.format {
      ...
      ImageFormat::Single => deck::ImageOptions::Regular { columns: 1, inline_sideboard: true },
      ImageFormat::Square => deck::ImageOptions::Regular { columns: 2, inline_sideboard: false },
      ImageFormat::Wide => deck::ImageOptions::Regular { columns: 3, inline_sideboard: false },
    };
